...
class ProgressHud10State extends State<ProgressHud10> {
  ...
  int intervalCount = 0;//转一圈定时器执行次数
  int step = 0;
  ...
  void initState() {
    ...
    this.timer = Timer.periodic(new Duration(milliseconds: this.speed), (timer) {
      setState(() {
        ...
        intervalCount = 1000 ~/ this.speed;
        step ++;
        if (step > (1000 / this.speed * count)) {
          step = 0;
        }


        for (int i = 1; i < this.count + 1; i++) {
          double tem = step - i * intervalCount / count;
          
          if (step >= (i + 1) * intervalCount) {
            w = (intervalCount * 0.5).toDouble();
            print('111111111 == $w,tem === $tem, step == $step, i = $i, intervalCount = $intervalCount');
          } else {
            print('222222222 == $w,tem === $tem, step == $step, i = $i');
            w = (tem * 0.5).toDouble();
          }
